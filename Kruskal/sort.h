#ifndef _MERGE_SORT_
#define _MERGE_SORT
/* need to make this work with for sorting the adjacency list's sub-list of vertices */
/* needs fair bit of tweaking. For one, buffers are not int*, rather list* which has an int, and sub-list needs to be sorted out based on that */

/* is it possible to write the sort function in C++ using templates so that it can accept different types, and call that function from C source file ? Mixing C & C++ */
